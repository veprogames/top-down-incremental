...
var health_bar_fill_rect_current: Rect2 = Rect2(health_bar_fill_rect)
...
func _ready() -> void:
	...
	health_bar_base_rect = health_bar_fill_rect.grow(2.0)
	
	enemy.current_hp_changed.connect(_on_enemy_current_hp_changed)


func _process(delta: float) -> void:
	health_bar_alpha = clampf(health_bar_alpha - delta * 0.8, -2.0, 2.0)
